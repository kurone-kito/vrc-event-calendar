...
- Does NOT contain any
  `<!-- vrc-event-calendar-roadmap-id: … -->` marker (the issue
  is not itself a roadmap).
- Does NOT contain any
  `<!-- vrc-event-calendar-blocked-by: … -->` marker.
...
**A0-T** (the scoped `vrc-event-calendar-roadmap-id` lookup
needed to resolve the explicit target's
`vrc-event-calendar-blocked-by` markers),
...
for `vrc-event-calendar-roadmap-id` to resolve
`vrc-event-calendar-blocked-by` dependency markers; see A2 for
...
- **A0-T only**: the scoped body-content lookup needed to resolve
  `vrc-event-calendar-blocked-by` markers on the explicit target.
...
- **A0-O only** (when `issue-scope` is `orphan-first`): a repo-wide
  ...
  `vrc-event-calendar-roadmap-id` or
  `vrc-event-calendar-blocked-by` markers.
...
- **A3 only**: a body-content search (e.g.,
  ...
  `vrc-event-calendar-roadmap-id` marker when checking
  `vrc-event-calendar-blocked-by` dependency markers (see A3
...
- All dependency issues are closed or otherwise completed. Check both
  ...
  `<!-- vrc-event-calendar-blocked-by: {roadmap-id} -->` markers
  — for each `{roadmap-id}`, find the issue whose body contains
  `<!-- vrc-event-calendar-roadmap-id: {roadmap-id} -->`. If that
...
3. **A3 filtered to zero** (A2 found candidates but all were filtered
   ...
   **Diagnostic — all candidates blocked by an open roadmap**: if every
   ...
   `<!-- vrc-event-calendar-blocked-by: X -->` marker points to a
...
- **Roadmap identity** (`vrc-event-calendar-roadmap-id`): placed
...
- **Sequential dependency** (`vrc-event-calendar-blocked-by`):
...
**Do not use `vrc-event-calendar-blocked-by` to group sub-tasks
