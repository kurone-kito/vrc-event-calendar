...
### Trusted Marker Actor Placeholder
...
| Placeholder role | Meaning | Example |
| --- | --- | --- |
| repository name placeholder | Repository short name used in worktree examples | `my-app` |
| marker prefix placeholder | Hidden issue-body marker prefix | `my-app` |
| trusted marker actor placeholder | Single JSON-escaped login allowed to post trusted markers | `trusted-user-a` |
| fix-validate command placeholder | Auto-fix plus validate command row | `npm run lint:fix && npm run lint` |
| pre-push-validate command placeholder | Non-mutating verify command row | `npm run lint && npm run test` |
| post-fix-validate command placeholder | Post-fix validate command row | `npm run lint:fix && npm test` |
| install-deps command placeholder | Dependency install command, or `true` when unnecessary | `npm install` |
...
Keep the resolved `install-deps` command safe to rerun across retries,
takeovers, and recreated worktrees.

## Marker prefix notes

`vrc-event-calendar` appears in two hidden issue-body markers:

- roadmap identity marker:
  `<!-- vrc-event-calendar-roadmap-id: {unique-id} -->`
- blocked-by marker:
  `<!-- vrc-event-calendar-blocked-by: {roadmap-id} -->`
...
seven placeholders above globally. Then verify that no unresolved
double-brace placeholder tokens remain in the copied files.
